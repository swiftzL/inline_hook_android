...
void hook(void* oldfun,void* newfun) {
    uint8_t *target = (uint8_t *)newfun;
    uint8_t backup[16]; // 备份空间
    // 备份原始指令
    memcpy(backup, target, sizeof(backup));
    // 5字节的跳转指令：br x16
    uint32_t branch_instruction = 0xd63f0000 | ((target - (target + 5)) >> 2) & 0x7ffff;
